...
var attrs = {
  ...
  convertAttr: function convertAttr (ast, log) {
    ...
    Object.keys(attrsMap).map(function (key) {
      var val = attrsMap[key].replace(/['"]/g, "'"); // 替换掉"" ''为 ''
  ...
  event: function event (key, val, attrs, tag) {
    ...
    var m = val.match(/\((.+?)\)$/);
    if (m && m.length > 0) {
      attrs[("data-" + event + "-attr")] = '{{[' + m[1].split(',').map(function (v) {
        v = v.trim();
  ...
  bind: function bind$$1 (key, val, attrs, tag, isIf) {
...
function convertAst (node, options, util) {
  ...
  if (tagConfig.virtualTag.indexOf(tagName) < 0) {
    wxmlAst.staticClass = staticClass ? ("" + staticClass).replace(/\"/g, '') : undefined;
  } else {
...
function compileToWxml (compiled, options) {
  ...
  // code = `<template${nameAttr}>${code}</template>`

  // 生成 slots code
  // Object.keys(slots).forEach(k => {
  //   const slot = slots[k]
  //   const row = generate(slot.node, options) || ''
  ...
  //       const code = handleExpression(row, expression, slot.depth)
  //       return code
  //     } else {
  //       return expression
  //     }
  ...
  return { code: code, compiled: compiled, slots: slots, /*importCode*/ }
}
